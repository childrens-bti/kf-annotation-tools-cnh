...
curl -sL -o "${DOWNLOADED_FILE}" "${GNOMAD_URL}"
...
docker run --rm -v $PWD:/work -v /home/ubuntu/kf-germline-workflow-cnh/data/references:/refs -w /work \
  pgc-images.sbgenomics.com/d3b-bixu/vcfutils:latest \
  bash -c "
    bcftools annotate --threads 12 \
